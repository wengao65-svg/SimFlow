#!/usr/bin/env node
/**
 * Publish dist/claude-marketplace to the claude-marketplace branch.
 */
...
const DIST = path.join(ROOT, 'dist', 'claude-marketplace');
const BRANCH = process.env.SIMFLOW_CLAUDE_MARKETPLACE_BRANCH || 'claude-marketplace';
const REMOTE = process.env.SIMFLOW_CLAUDE_MARKETPLACE_REMOTE || 'origin';
...
function ensureMarketplaceDist() {
  if (!SKIP_BUILD) {
    run('npm', ['run', 'build:claude-marketplace']);
  }
  const marketplace = path.join(DIST, '.claude-plugin', 'marketplace.json');
  const plugin = path.join(DIST, 'plugins', 'simflow', '.claude-plugin', 'plugin.json');
  if (!fs.existsSync(marketplace) || !fs.existsSync(plugin)) {
    throw new Error('dist/claude-marketplace is missing required marketplace files');
  }
}
...
function publish() {
  ...
  const worktree = fs.mkdtempSync(path.join(os.tmpdir(), 'simflow-claude-marketplace-'));
  try {
    ...
    const version = JSON.parse(fs.readFileSync(path.join(DIST, 'plugins', 'simflow', '.claude-plugin', 'plugin.json'), 'utf-8')).version;
    run('git', ['commit', '-m', `Publish SimFlow Claude marketplace ${version}`], { cwd: worktree });
    if (DRY_RUN) {
