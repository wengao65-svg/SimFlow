#!/usr/bin/env python3
"""Parse Quantum ESPRESSO output files."""

import argparse
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))
from lib.parsers.qe_parser import QEParser


def main():
    parser = argparse.ArgumentParser(description="Parse QE output")
    parser.add_argument("file", help="QE output file (.out)")
    parser.add_argument("--check-convergence", action="store_true",
                        help="Check SCF convergence")
    args = parser.parse_args()

    qe = QEParser()

    try:
        if args.check_convergence:
            result = qe.check_convergence(args.file)
            print(json.dumps(result, indent=2))
        else:
            parsed = qe.parse(args.file)
            output = {
                "software": parsed.software,
                "job_type": parsed.job_type,
                "converged": parsed.converged,
                "total_energy": parsed.total_energy,
                "final_energy": parsed.final_energy,
                "parameters": parsed.parameters,
                "warnings": parsed.warnings,
                "errors": parsed.errors,
                "metadata": parsed.metadata,
            }
            print(json.dumps(output, indent=2))
    except Exception as e:
        print(json.dumps({"status": "error", "message": str(e)}))
        sys.exit(1)


if __name__ == "__main__":
    main()
